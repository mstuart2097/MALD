...
w_start[,1] <- lowess((y[,1]-mean(y[,1]))^2, f = 0.05)$y #informed starting values
w_start[,2] <- lowess((y[,2]-mean(y[,2]))^2, f = 0.05)$y #informed starting values
...
delta <- sample(c(0:3),T,replace=TRUE,prob=lambda)
